...
- `modules/aws/ses`'s send policy (`send_policy_arn`) only granted `ses:SendEmail`/`ses:SendRawEmail` on the domain identity's ARN, never on the configuration set's — SES enforces resource-level IAM on the configuration set named in `SendEmailCommand`'s `ConfigurationSetName` separately from the identity being sent from, so any consumer following `1.16.0`'s own documented usage (pass `configuration_set_name` to `ConfigurationSetName` on every send) got `AccessDenied` on every send once a configuration set existed, even with the identity grant present and correct. Confirmed live in a real consumer (`latb-fe`): a real subscriber-confirmation send through the deployed web Lambda's role failed with `AccessDenied: ... not authorized to perform: 'ses:SendEmail' on resource 'arn:aws:ses:...:configuration-set/lookatthesebirds-com-feedback'`. The send policy's `resources` list now includes the configuration set's ARN whenever one is created (`local.create_config_set`), alongside the identity's — `null` for a consumer with no feedback monitoring configured, so nothing changes for existing usage of the module without `feedback_notification_email`/`existing_topic_arn`. `context/features/ses-bounce-complaint-feedback.md`.
