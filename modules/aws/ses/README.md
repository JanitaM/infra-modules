...
Pass the module's `configuration_set_name` output to `SendEmailCommand`'s `ConfigurationSetName` — SES only fires bounce/complaint events for sends that reference the configuration set explicitly, and only fires them at all when the address behind `feedback_notification_email` has confirmed its SNS subscription (a confirmation email is sent on apply; nothing arrives until that link is clicked). `send_policy_arn` already covers this: once a configuration set exists, its ARN is included in the send policy's `Resource` list alongside the domain identity's, since SES enforces resource-level IAM on the configuration set separately from the identity being sent from.
